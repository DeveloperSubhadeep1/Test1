...
// Filter out @-names for the word-based blocklist, as they are handled by a generic regex.
// The remaining words are lowercased for case-insensitive matching.
const WORD_BLOCKLIST = RAW_CHANNEL_NAMES
    .filter(name => !name.startsWith('@'))
    .map(name => name.toLowerCase());

// A regex to match any of the specific non-@ words from our blocklist (e.g., MovieWorldHub)
const BLOCKLIST_REGEX = new RegExp(`\\b(${WORD_BLOCKLIST.join('|')})\\b`, 'gi');
...
function coreFilenameParser(filename: string): { moviename: string; year: string | null; languages: string[]; quality: string | null; size: string | null; season: number | null, episode: number | null } {
    ...
    let cleanedName = nameWithoutExt
        .replace(/@[\w-]+/gi, '')       // Stage 1: Remove @-tags like @ClipmateMovies. Note: Removed '.' from character class to prevent over-matching.
        .replace(/\[.*?\]/g, '')         // Stage 2: Remove bracketed tags like [TGx].
        .replace(BLOCKLIST_REGEX, '');   // Stage 3: Remove other blocklisted words.
